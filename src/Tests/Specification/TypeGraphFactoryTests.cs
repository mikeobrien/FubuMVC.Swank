...
namespace Tests.Specification
{
    ...
    public class TypeGraphFactoryTests
    {
        ...
        public class SampleValuePostHandler
        {
            public void Execute(ComplexTypeWithSampleValue request) { }
        }
        ...
        {
            var member = should_be_complex_type(CreateFactory().BuildGraph(
                typeof(ComplexTypeWithSampleValue)), 1)
                .Members.Single();
        ...
        {
            var action = Behavior.BuildGraph().AddAndGetAction<SampleValuePostHandler>();

            var member = should_be_complex_type(CreateFactory().BuildGraph(
                typeof(ComplexTypeWithSampleValue), action), 1)
                .Members.Single();
        ...
        {
            public ShortNamespaceComplexType Properties { get; set; }
        }

        public class ShortNamespaceComplexType
        {
            public ShortNamespaceList Properties { get; set; }
        }
        ...
        public void should_create_unique_short_namespace()
        {
            ...
            var member = type.Members.Member("Properties");

            member.Type.ShortNamespace.ToArray()
                .ShouldEqual(new List<string> { "ShortNamespaceRoot" }.ToArray());

            member = member.Type.Members.Member("Properties");

            member.Type.ShortNamespace.ToArray()
                .ShouldEqual(new List<string> { "Properties" }.ToArray());
        }
    }
